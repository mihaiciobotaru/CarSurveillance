...
class ImageProcessor:
    ...
    # The rectangle coordinates for the parking spaces in the image
    ...
    def check_parking_spaces(image: np.ndarray, car_points: list[Point], display_intermediate=False) -> list[bool]:
        ...
        for point in car_points:
            if ImageProcessor.PARKING_BOX.check_point_inside(point):
                ...
                ImageProcessor.logger.debug(f"{point} is inside the parking box.")
            else:
                ImageProcessor.logger.debug(f"{point} is outside the parking box, skipping it.")
        ...
        ImageProcessor.logger.debug("Applying perspective transformation to the image.")
        src_points = np.float32([
            ImageProcessor.TOP_RIGHT.to_tuple(), ImageProcessor.BOTTOM_RIGHT.to_tuple(), \
            ImageProcessor.TOP_LEFT.to_tuple(), ImageProcessor.BOTTOM_LEFT.to_tuple()
        ])
        dst_points = np.float32([(0, 0), (1000, 0), (0, 1000), (1000, 1000)])
        ...
        rotated_warped_image = ImageUtils.rotate(warped_image, angle=-90)
        ...
        ImageProcessor.logger.debug("Translating car points to the warped image coordinates.")
        translated_car_points = []
        for point in car_points_inside_box:
            src_point = np.float32([[[point.x, point.y]]])
            dst_point = cv2.perspectiveTransform(src_point, warp_matrix)
            new_point = Point(dst_point[0][0][0], dst_point[0][0][1])
            new_point = ImageUtils.rotate_point_against_image(new_point, warped_image, angle=-90)
            translated_car_points.append(new_point)
            
        # Draw and display the car points in the warped image of parking spaces
        if display_intermediate:
            ImageProcessor.logger.debug("Drawing translated car points on the rotated warped image.")
        ...
        ImageProcessor.logger.debug("Checking parking spaces based on translated car points.")
        parking_spaces = []
